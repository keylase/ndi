...
struct decklink_ndi_bridge : public IDeckLinkInputCallback
{
	...
	{	// Get the new frame rate
		...
		const long width = newDisplayMode->GetWidth();
		const long height = newDisplayMode->GetHeight();
		...
		BMDFieldDominance field_dom = newDisplayMode->GetFieldDominance();
		switch (field_dom)
...
private:
	...
	{	NDIlib_video_frame_v2_t ndi_video_frame = { };
		...
		// Release the previously stored video frame
		if (m_p_decklink_video_frame)
			m_p_decklink_video_frame->Release();
